import {
  StyleSheet, TextInput, ScrollView, Text, View, TouchableOpacity, StatusBar, Alert,
} from 'react-native';
import React,{useState} from 'react';
import axios from 'axios';
import Slogo from '../../assets/img/splashlogo.svg';
import COLORS from '../../consts/colors';
import { fontPixel, pixelSizeVertical, pixelSizeHorizontal, widthPixel, heightPixel, } from '../../utils/ResponsiveStyle';
  ...
  const dispatch =useDispatch()
   ...
   const doEmail= async()=>{
    if (text.trim() === '') {
      ...
      try {
        const response = await axios.post('http://192.168.50.2:8000/auth/sendemail',{ "email":text});
  ...
  return (
    <View style={styles.main}>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <TextInput onChangeText={onChangeText} value={text}  onFocus={()=>{setIsRequired(false)}}  style={styles.input} placeholder="Enter your email"  />
        {isRequired && <Text style={{ color: COLORS.secondary,fontSize:12}}>Email is required.</Text>}
