...
const ParkingSlots = () => {
  const {showLoader,parkingSlots}=useSelector((state:any)=>state.adminSlice)
    ...
    <ActivityIndicator size={'large'} color={COLORS.white}></ActivityIndicator>
      ...
      {parkingSlots.map((item:any, index:number) => (
        <View style={styles.container} key={index}>
          ...
          <Text style={styles.text}>Parking Lot Name: {item.parkingLotName}</Text>
          <Text style={styles.text}>Parking Space Number: {item.parkingSpaceNumber}</Text>
          <Text style={styles.text}>Entry Time: {item.entryTime}</Text>
          <Text style={styles.text}>Booked Time: {item.bookedTime}</Text>
          <Text style={styles.text}>Status: {item.booked ? 'Booked' : 'Not Booked'}</Text>
